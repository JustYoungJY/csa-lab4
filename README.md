...
При появлении данных во входном буфере выставляется сигнал IRQ. Если разрешение прерываний включено (`IE = 1`), в начале следующего цикла выборки процессор:
...
| Опкод | Мнемоника      | Операнд | Действие | Описание                             |
|:-----:|:---------------|:-------:|:---------|:-------------------------------------|
| 0x01  | `PUSH`         |  `imm`  | `DS.push(imm)` | Положить 24-битную константу на стек |
| 0x02  | `POP`          |    –    | `DS.pop()` | Удалить TOS                          |
| 0x03  | `LOAD`         |    –    | `addr=pop(); push(MEM[addr])` | Косвенное чтение из памяти           |
| 0x04  | `STORE`        |    –    | `v=pop(); a=pop(); MEM[a]=v` | Косвенная запись в память            |
| 0x05  | `ADD`          |    –    | `b=pop(); a=pop(); push(a+b); C,V` | Сложение; обновляет C и V            |
| 0x06  | `SUB`          |    –    | `b=pop(); a=pop(); push(a-b); C,V` | Вычитание; обновляет C и V           |
| 0x07  | `MUL`          |    –    | `b=pop(); a=pop(); push((a*b) & M)` | Умножение, нижние 32 бита            |
| 0x08  | `DIV`          |    –    | `b=pop(); a=pop(); push(trunc(a/b))` | Деление к нулю                       |
| 0x09  | `MOD`          |    –    | `b=pop(); a=pop(); push(a % b)` | Остаток от деления                   |
| 0x0A  | `CMP`          |    –    | `b=pop(); a=pop(); push(1 if a==b else 0)` | Равенство                            |
| 0x0B  | `GT`           |    –    | `b=pop(); a=pop(); push(1 if a>b else 0)` | Больше (знаковое)                    |
| 0x0C  | `LT`           |    –    | `b=pop(); a=pop(); push(1 if a<b else 0)` | Меньше (знаковое)                    |
| 0x0D  | `JMP`          | `addr`  | `PC = addr` | Безусловный переход                  |
| 0x0E  | `JZ`           | `addr`  | `if pop()==0: PC=addr` | Переход если TOS == 0                |
| 0x0F  | `CALL`         | `addr`  | `RS.push(PC); PC=addr` | Вызов функции                        |
| 0x10  | `RET`          |    –    | `PC = RS.pop()` | Возврат из функции                   |
| 0x11  | `IRET`         |    –    | `PC=RS.pop(); IE=1` | Возврат из прерывания                |
| 0x12  | `HALT`         |    –    | останов | Остановить тактовый генератор        |
| 0x13  | `PUSH_M`       | `addr`  | `DS.push(MEM[addr])` | Прямое чтение из памяти на стек      |
| 0x14  | `POP_M`        | `addr`  | `MEM[addr] = DS.pop()` | Запись TOS в память напрямую         |
| 0x15  | `ADC`          |    –    | `b=pop(); a=pop(); push(a+b+C); C,V` | Сложение с переносом                 |
| 0x16  | `SBB`          |    –    | `b=pop(); a=pop(); push(a-b-C); C,V` | Вычитание с заёмом                   |
| 0x17  | `MUL64`        |    –    | `b=pop(); a=pop(); push(lo); push(hi)` | Умножение 32x32 -> 64    |
| 0x18  | `PUSH_CARRY`   |    –    | `DS.push(C)` | Флаг C на стек                       |
| 0x19  | `PUSH_OVERFLOW`|    –    | `DS.push(V)` | Флаг V на стек                       |
| 0x1A  | `STI`          |    –    | `IE = 1` | Разрешить прерывания                 |
...
Симулятор воспроизводит поведение процессора с точностью до такта (`tick`). Каждая инструкция занимает ровно **2 такта**: один на выборку (`Fetch`), один на исполнение (`Execute`). Обработка прерывания добавляет 1 такт.
...
```
1. Переопределить IRQ по состоянию input_buffer
      irq <- bool(input_buffer)
2. Если IE=1 и IRQ=1 -> обработать прерывание:
      RS.push(PC); PC <- 0; IE <- 0; IRQ <- 0; tick()
3. Fetch: MEM[PC] -> decode -> PC++; tick()
4. Execute: выполнить инструкцию; tick()
```

**Обработка прерывания** происходит на границе инструкций, что гарантирует атомарность исполнения. Флаг `IE` сбрасывается при входе в ISR и восстанавливается `IRET`, предотвращая вложенные прерывания.
...
Символ поступает в аппаратный входной буфер не раньше указанного такта.

### Структура golden-тестов

Каждому тесту соответствует файл `golden/<name>.yaml`, содержащий:

- **`machine_code`** – дамп машинного кода, сгенерированный транслятором
- **`input_schedule`** – расписание ввода
- **`expected_output`** – эталонный вывод процессора
- **`processor_trace`** – журнал работы процессора
